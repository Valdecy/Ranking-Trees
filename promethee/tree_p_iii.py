...
from promethee.util_p_iii import promethee_iii, genetic_algorithm, rank

###############################################################################

# Main    
def tree_promethee_iii(dataset, target_assignment = [], W = [], Q = [], P = [], S = [], F = [], lmbd = [], elite = 1, eta = 1, mu = 2, population_size = 25, mutation_rate = 0.01, generations = 150, samples = 0.70, number_of_models = 100):
    count           = 0
    ensemble_model  = []
    ...
    def target_function(variable_list):
        ...
        L_t = variable_list[-1]
        p_iii = promethee_iii(dataset = random_dataset, W = W_t, Q = Q_t, S = S_t, P = P_t, F = F_t, lmbd = L_t)
        p_iii = rank(p_iii)
        kendall_tau, _ = stats.kendalltau(random_y, p_iii)
    ...
    while count < number_of_models:
        ...
        for i in range(0, len(S_ga)):
            if (F_ga[i] != 't7'):
                S_ga[i] = 0
        kendall_tau = ga[-1]*(-1)
        y_hat = promethee_iii(dataset = random_dataset, W = W_ga, Q = Q_ga, S = S_ga, P = P_ga, F = F_ga, lmbd = L_ga) 
        y_hat = rank(y_hat)
...
# Prediction
def predict_p_iii(models, dataset, verbose = True):
    prediction     = []
    ...
    for i in range(0, len(ensemble_model)):
        ...
        rank_ = promethee_iii(dataset = alternatives, W = ensemble_model[i][0], Q = ensemble_model[i][7], S = ensemble_model[i][9], P = ensemble_model[i][8], F = ensemble_model[i][10], lmbd = ensemble_model[i][11])
        rank_ = rank(rank_)
        for j in range(0, len(solutions)):
            if (i == 0):
                solutions[j] = [rank_[j]]
            else:
                solutions[j].extend([rank_[j]])
    for i in range(0, dataset.shape[0]):
       prediction.append(int(max(set(solutions[i]), key = solutions[i].count)))
       if (verbose == True):
           print('a' + str(i + 1) + ' = ' + str(prediction[i] + 1))
