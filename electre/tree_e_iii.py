...
from electre.util_e_iii import electre_iii, genetic_algorithm

###############################################################################

# Main    
def tree_electre_iii(dataset, target_assignment = [], W = [], P = [], Q = [], V = [], elite = 1, eta = 1, mu = 2, population_size = 25, mutation_rate = 0.01, generations = 150, samples = 0.70, number_of_models = 100):
    count           = 0
    ensemble_model  = []
    ...
    def target_function(variable_list):
        ...
        _, _, _, _, e_iii, _ = electre_iii(dataset = random_dataset, W = W_t, Q = Q_t, P = P_t, V = V_t, graph = False)
        e_iii = e_iii[:,0]
        e_iii = e_iii.tolist()
        kendall_tau, _ = stats.kendalltau(random_y, e_iii)
    ...
    while count < number_of_models:
        ...
        kendall_tau = ga[-1]*(-1)
        _, _, _, _, y_hat, _ = electre_iii(dataset = random_dataset, W = W_ga, Q = Q_ga, P = P_ga, V = V_ga, graph = False)
        y_hat = y_hat[:,0]
        y_hat = y_hat.tolist()
        ensemble_model.append([W_ga, kendall_tau, criteria, criteria_remove, cases_remove, y_hat, random_y, Q_ga, P_ga, V_ga])
        count = count + 1
...
# Prediction
def predict_e_iii(models, dataset, verbose = True):
    prediction     = []
    ...
    for i in range(0, len(ensemble_model)):
        ...
        _, _, _, _, rank, _ = electre_iii(dataset = alternatives, W = ensemble_model[i][0], Q = ensemble_model[i][7], P = ensemble_model[i][8], V = ensemble_model[i][9], graph = False)
        rank = rank[:,0]
        rank = rank.tolist()
...
def metrics_e_iii(models):
    ...
    return mean_features, std_features, kdl_mean, kdl_std, q_tresholds_mean, q_tresholds_std, p_tresholds_mean, p_tresholds_std, v_tresholds_mean, v_tresholds_std
