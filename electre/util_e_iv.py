...
def clip_thresholds(population, indv = 0, size = 3, min_values = [-5,-5], max_values = [5,5]):
    ...
    for i in range(0, size):
        if (population[indv][size*0 + i] > population[indv][size*1 + i]):
            population[indv][size*0 + i] = population[indv][size*1 + i]
        if (population[indv][size*1 + i] > population[indv][size*2 + i] and population[indv][size*0 + i] > population[indv][size*2 + i]):
            population[indv][size*0 + i] = population[indv][size*2 + i]
...
def electre_iv(dataset, P, Q, V, graph = False):
    ...
    if (graph == True):
        po_ranking(rank_P)
    return credibility, rank_D, rank_A, rank_N, rank_P
