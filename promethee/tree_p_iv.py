...
from promethee.util_p_iv import promethee_iv, genetic_algorithm

###############################################################################

# Main    
def tree_promethee_iv(dataset, target_assignment = [], W = [], Q = [], P = [], S = [], F = [], elite = 1, eta = 1, mu = 2, population_size = 25, mutation_rate = 0.01, generations = 150, samples = 0.70, number_of_models = 100):
    count           = 0
    ensemble_model  = []
    ...
    while count < number_of_models:
        ...
        for i in range(0, len(S_ga)):
            if (F_ga[i] != 't7'):
                S_ga[i] = 0
        kendall_tau = ga[-1]*(-1)
        y_hat = promethee_iv(dataset = random_dataset, W = W_ga, Q = Q_ga, S = S_ga, P = P_ga, F = F_ga)
        y_hat = y_hat[:,0]
        y_hat = y_hat.tolist()
...
# Prediction
def predict_p_iv(models, dataset, verbose = True):
    prediction     = []
    ...
    for i in range(0, len(ensemble_model)):
        ...
        rank = promethee_iv(dataset = alternatives, W = ensemble_model[i][0], Q = ensemble_model[i][7], S = ensemble_model[i][9], P = ensemble_model[i][8], F = ensemble_model[i][10])
        rank = rank[:,0]
        rank = rank.tolist()
