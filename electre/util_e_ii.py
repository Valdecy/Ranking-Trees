...
def clip_parameters(population, indv = 0, min_values = [-5,-5], max_values = [5,5]):
    if (population[indv][-2] < population[indv][-3]):
        population[indv][-2] = population[indv][-3] 
    if (population[indv][-5] < population[indv][-6]):
        population[indv][-5] = population[indv][-6]          
    if (population[indv][-4] < population[indv][-5] and population[indv][-4] < population[indv][-6]):
        population[indv][-4] = population[indv][-6]  
        population[indv][-5] = population[indv][-6]  
    if (population[indv][-4] < population[indv][-5] and population[indv][-4] > population[indv][-6]):
        population[indv][-4] = population[indv][-5]  
...
def initial_population(population_size = 5, min_values = [-5,-5], max_values = [5,5], target_function = target_function):
    ...
    for i in range(0, population_size):
        ...
        population = clip_parameters(population, indv = i, min_values = min_values, max_values = max_values)
...
def breeding(population, fitness, min_values = [-5,-5], max_values = [5,5], mu = 1, elite = 0, target_function = target_function):
    ...
    for i in range (elite, offspring.shape[0]):
        ...
        offspring = clip_parameters(offspring, indv = i, min_values = min_values, max_values = max_values)
...
def electre_ii(dataset, W, c_minus = 0.65, c_zero = 0.75, c_plus = 0.85, d_minus = 0.25, d_plus = 0.50, graph = False):
    ...
    if (graph == True):
        po_ranking(rank_P)
    return concordance, discordance, dominance_s, dominance_w, rank_D, rank_A, rank_N, rank_P
